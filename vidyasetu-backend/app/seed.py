...
def run():
    ...
    # ---------------- A couple of students at BITS Pilani, so Dr. Kavita Sharma's
    # faculty "My Students" view (scoped to her own institution) isn't empty ----------------
    for name in ["Neha Kapoor", "Arjun Singh"]:
        email = name.lower().replace(" ", ".") + "@student.demo"
        u = m.User(email=email, hashed_password=hash_password(DEMO_PASSWORD), role=m.RoleEnum.student, full_name=name)
        db.add(u)
        db.flush()
        sp = m.StudentProfile(user_id=u.id, institution_id=bits.id, department_id=cse_bits.id,
                               program="B.E. Computer Science", year_of_study=random.choice([2, 3, 4]),
                               cgpa=round(random.uniform(7.0, 9.4), 1), target_career_role_id=ai_ml_role.id)
        db.add(sp)
        db.flush()
        for sk in [python, ml, sql, docker]:
    ...
    # ---------------- Assessments (real, scored questions — see app/routers/assessments.py) ----------------
    def q(text, options, correct_index):
        return {"id": str(uuid.uuid4()), "text": text, "options": options, "correct_index": correct_index}

    db.add(m.Assessment(
        skill_id=python.id, title="Python Fundamentals Assessment", assessment_type="Technical Assessment",
        questions=[
            q("What does `len([1,2,3])` return?", ["2","3","4","Error"], 1),
            q("Which keyword defines a function in Python?", ["func","def","function","lambda"], 1),
            q("What is the output of `type(5/2)` in Python 3?", ["<class 'int'>","<class 'float'>","<class 'str'>","Error"], 1),
            q("Which of these creates an empty dictionary?", ["dict()","{}","Both A and B","[]"], 2),
            q("What does `except` handle in Python?", ["Loops","Exceptions","Imports","Classes"], 1),
        ],
    ))
    db.add(m.Assessment(
        skill_id=docker.id, title="Docker Fundamentals Assessment", assessment_type="Technical Assessment",
        questions=[
            q("Which file defines how a Docker image is built?", ["docker-compose.yml","Dockerfile","image.json","build.sh"], 1),
            q("What command lists running containers?", ["docker ps","docker ls","docker list","docker show"], 0),
            q("What is the purpose of a Docker volume?", ["Networking","Persistent storage","CPU limits","Image compression"], 1),
            q("Which command builds an image from a Dockerfile?", ["docker run","docker build","docker create","docker start"], 1),
        ],
    ))
    db.add(m.Assessment(
        skill_id=sql.id, title="SQL Advanced Queries Assessment", assessment_type="Technical Assessment",
        questions=[
            q("Which clause filters rows after grouping?", ["WHERE","HAVING","FILTER","GROUP BY"], 1),
            q("Which JOIN returns all rows from both tables, matched where possible?", ["INNER JOIN","LEFT JOIN","FULL OUTER JOIN","CROSS JOIN"], 2),
            q("What does `COUNT(*)` return?", ["Sum of a column","Number of rows","Number of columns","Distinct values"], 1),
            q("Which keyword removes duplicate rows from results?", ["UNIQUE","DISTINCT","FILTER","GROUP"], 1),
        ],
    ))
    db.add(m.Assessment(
        skill_id=ml.id, title="Machine Learning Fundamentals Assessment", assessment_type="Technical Assessment",
        questions=[
            q("What is overfitting?", ["Model too simple","Model memorizes training data, generalizes poorly","Model trains too fast","Not enough features"], 1),
            q("Which of these is a supervised learning task?", ["Clustering","Classification","Dimensionality reduction","Anomaly detection"], 1),
            q("What does a confusion matrix show?", ["Loss over time","Prediction errors by class","Feature importance","Learning rate"], 1),
            q("What is the purpose of a train/test split?", ["Speed up training","Evaluate generalization","Reduce dataset size","Remove outliers"], 1),
        ],
    ))
    db.commit()
    db.close()
