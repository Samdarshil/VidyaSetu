...
@router.post("/me/github-import", response_model=GitHubImportResponse)
def import_github_evidence(
    payload: GitHubImportRequest,
    db: Session = Depends(get_db),
    user: User = Depends(require_role("student")),
):
    """Records real GitHub repo evidence for skills the platform already
    tracks. The repo list is fetched CLIENT-SIDE from GitHub's public REST
    API (no token, no secret — public repos only) and handed to us here to
    persist as evidence. This is the demo-safe connection flow: no OAuth
    app registration needed, nothing invented, and the evidence type is
    explicit about its (weaker, self-reported-adjacent) weight rather than
    claiming expertise from a repo's mere existence.
    """
    profile = _get_own_student_profile(db, user)

    # Map common GitHub "primary language" values to platform skill names.
    LANGUAGE_TO_SKILL = {
        "python": "Python", "javascript": "React", "typescript": "TypeScript",
        "jupyter notebook": "Machine Learning", "dockerfile": "Docker",
        "sql": "SQL", "plpgsql": "SQL", "html": "React",
    }

    updated_skills = set()
    for repo in payload.repos:
        lang_key = (repo.primary_language or "").strip().lower()
        skill_name = LANGUAGE_TO_SKILL.get(lang_key)
        if not skill_name:
            continue  # no confident mapping for this language — skip rather than guess

        skill = db.query(Skill).filter(Skill.name == skill_name).first()
        if not skill:
            continue  # only attach evidence to skills VidyaSetu already tracks
        ...
        # Public-repo evidence is real but weaker than a graded assessment —
        # score_contribution reflects "a project exists using this language,"
        # not a claim of mastery. See EVIDENCE_WEIGHTS in engine.py: Project
        # evidence is weighted 1.1x but this contributes a modest 55 on its
        # own, so one repo alone won't inflate a score.
        db.add(SkillEvidence(
            student_skill_id=student_skill.id,
            evidence_type="Project",
            score_contribution=55.0,
            source_note=f"GitHub repo: {repo.name} ({repo.primary_language})",
        ))
        db.flush()
        recompute_student_skill(db, student_skill)
    ...
    return GitHubImportResponse(
        ...
        repos_processed=len(payload.repos),
        skills_updated=sorted(updated_skills),
    )
